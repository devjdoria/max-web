...
export default function AdminPortal() {
  ...
  async function load() {
    ...
    try {
      ...
      if (!accessResponse.ok)
        throw new Error(
          'No se pudo comprobar el acceso. Revisa las variables de Vercel.',
        );
      ...
      setLoadError(
        cause instanceof Error ? cause.message : 'No se pudo cargar el portal.',
      );
    } finally {
      setLoading(false);
    }
  ...
  async function unlock(e: React.SyntheticEvent<HTMLFormElement>) {
    ...
    const r = await fetch('/api/access', {
      ...
      body: JSON.stringify({ date: d.get('date') }),
    });
    if (!r.ok) {
      setError('Esa no es nuestra fecha.');
      return;
    }
  ...
  if (loading)
    return (
      <main className="admin-loading">
        <div>
          <LoaderCircle />
          <span>Preparando vuestro portal…</span>
        </div>
      </main>
    );
  if (loadError)
    return (
      <main className="admin-error">
        <div>
          <span>Algo falta por conectar</span>
  ...
  const textFields = [
    ['hero_eyebrow', 'Texto superior'],
    ['hero_pretitle', 'Dedicatoria corta'],
    ['hero_title', 'Título'],
    ['hero_name', 'Nombre'],
    ['hero_description', 'Texto principal'],
    ['story_kicker', 'Etiqueta de la historia'],
    ['story_title', 'Título de la historia'],
    ['story_description', 'Descripción de la historia'],
    ['love_note', 'Carta de amor'],
    ['footer_text', 'Texto final'],
  ];
  return (
    <main className="admin-page">
      ...
      <section className="admin-shell">
        <aside>
          <a href="#portada">Portada y textos</a>
          <a href="#sorpresas">Sorpresas</a>
        ...
        <div className="admin-content">
          ...
          <section id="portada">
            ...
            <div className="admin-fields">
              {textFields.map(([key, label]) => (
                <label key={key}>
                  {label}
                  {key.includes('description') || key === 'love_note' ? (
                    <Textarea
                      value={content[key] || ''}
          ...
          <section id="sorpresas">
            <p className="section-kicker">El rincón secreto</p>
            <h2>Sorpresas</h2>
            <div className="admin-surprises">
              {surprises.map((s, i) => (
                <article key={s.id || i}>
                  <Sparkles />
                  <label>
                  ...
                  <label className="admin-check">
                    <input
                      ...
                      onChange={(e) =>
                        setSurprises((a) =>
                          a.map((x, n) =>
                            n === i ? { ...x, locked: e.target.checked } : x,
                          ),
                        )
                      }
                    />{' '}
