...
function present(row: Row) {
  ...
  return {
    ...
    mediaUrl: row.media_path
      ? supabase.storage.from(BUCKET).getPublicUrl(row.media_path).data
          .publicUrl
      : undefined,
...
function readFields(data: FormData) {
  return {
    ...
    memory_date: formText(data, 'date'),
...
function valid(fields: ReturnType<typeof readFields>) {
  return (
    ...
    ['viaje', 'momento'].includes(fields.category) &&
    /^\d{4}-\d{2}-\d{2}$/.test(fields.memory_date)
  );
}
...
export async function GET() {
  try {
    ...
    return Response.json(
      { error: 'No se pudieron cargar los recuerdos' },
      { status: 500 },
    );
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { data: row, error } = await supabase
      ...
      .single();
    if (error) throw error;
    return Response.json(present(row as Row), { status: 201 });
  } catch {
    return Response.json(
      { error: 'No se pudo guardar el recuerdo' },
      { status: 500 },
    );
  }
}
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    const { data: row, error } = await supabase
      ...
      .single();
    if (error) throw error;
    return Response.json(present(row as Row));
  } catch {
    return Response.json(
      { error: 'No se pudo actualizar el recuerdo' },
      { status: 500 },
    );
  }
}
