...
function signature() { const secret = process.env.ACCESS_COOKIE_SECRET; if (!secret) throw new Error('Falta ACCESS_COOKIE_SECRET'); return createHmac('sha256',secret).update('maxime-access-v2').digest('hex'); }
...
export async function grantAccess() { (await cookies()).set(COOKIE,signature(),{httpOnly:true,sameSite:'lax',secure:process.env.NODE_ENV==='production',path:'/'}); }
